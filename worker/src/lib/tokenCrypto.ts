/**
 * AES-256-GCM token encryption — generic library used to store any
 * sensitive secret in D1 without leaking plaintext through DB dumps.
 *
 * Originally extracted from lib/ga4TokenCrypto.ts during Phase 3 PR 1
 * of the Traffic Impact data-depth roadmap. The GA4-specific shim
 * (ga4TokenCrypto.ts) re-exports these so existing call sites compile
 * unchanged.
 *
 * Output format and crypto parameters are byte-identical to the
 * pre-extract version: 12-byte random IV, 16-byte AES-GCM auth tag,
 * AES-256 key derived from a 64-char hex string. Existing ciphertexts
 * stored in D1 (e.g. ga4_connections.refresh_token_enc) decrypt
 * without migration.
 *
 * Error messages keep the "ga4TokenCrypto:" prefix for backward-
 * compatibility with cron logs. (A separate cleanup PR can rename to
 * "tokenCrypto:" once nothing depends on log-grep semantics.)
 */
