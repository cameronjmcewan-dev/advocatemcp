...
export async function handleGA4Start(request: Request, env: Env): Promise<Response> {
  ...
  // the GA4 read scope. handleGA4Callback consumes the resulting
  // id_token to extract the connected Google account's email and
  // persist it on the ga4_connections row. The dashboard surfaces it
  // on the GA4 card ("Connected as <email>") so admins can verify
  // which account currently holds the refresh token at a glance.
  // Mirror of the change in apiGA4StartLink (worker/src/routes/
  // portal.ts) — the new POST-based start endpoint that newer
  // dashboard JS uses; both paths must request the same scopes.
  authorizeUrl.searchParams.set("scope",         "openid email https://www.googleapis.com/auth/analytics.readonly");
...
export async function handleGA4Callback(request: Request, env: Env): Promise<Response> {
  ...
  const tokenJson = (await tokenRes.json()) as {
    ...
    access_token?: string;
    expires_in?: number;
    // Granted when the OAuth scope includes `openid email`. Carries
    // the connected Google account's email in its JWT payload.
    // Optional because tenants who completed OAuth before scope was
    // widened (migration 0027) still have a refresh token that only
    // grants the analytics.readonly scope — their id_token field is
    // undefined and the email field stays null until they reconnect.
    id_token?: string;
  };
  ...
  // Decode the connected Google account's email from id_token. JWT
  // signature is NOT verified here — the token came directly from
  // Google's /token endpoint over TLS, so the trust boundary is the
  // TLS connection, not the signature. Email is UI metadata only
  // (not security-bearing). Mirror of the GSC equivalent in
  // worker/src/routes/gscOauth.ts (PR #258).
  //
  // Best-effort: missing id_token, malformed payload, non-string
  // email all fall through with googleEmail=null. The row still
  // writes; the GA4 card just doesn't surface an email for that
  ...
  // tenants connected before migration 0027 the column is null and
  // the GA4 card falls back to its old "Connected" copy.
  await env.DB
    .prepare(
      "INSERT OR REPLACE INTO ga4_connections (slug, refresh_token_enc, status, connected_at, google_account_email) VALUES (?, ?, 'connected', datetime('now'), ?)",
    )
