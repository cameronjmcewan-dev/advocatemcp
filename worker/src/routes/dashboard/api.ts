...
function json(status: number, body: unknown): Response {
  return new Response(JSON.stringify(body), {
    ...
    headers: {
      ...
      "Access-Control-Allow-Origin":      "https://customers.advocatemcp.com",
...
export async function listDashboards(request: Request, env: Env): Promise<Response> {
  ...
  if (!biz) return json(404, { error: "No business found for this account" });
  ...
  return json(200, { business_slug: biz.slug, business_name: biz.business_name, dashboards: rows });
}
...
export async function getOneDashboard(
  ...
  if (!ctx) return json(401, { error: "Unauthorized" });
  const biz = await resolveBusiness(request, env, ctx);
  if (!biz) return json(404, { error: "No business found" });
  const id = Number.parseInt(idStr, 10);
  if (!Number.isFinite(id)) return json(400, { error: "invalid_id" });
  const row = await getDashboard(env.DB, id, ctx.user_id, biz.id);
  if (!row) return json(404, { error: "not_found" });
  return json(200, { dashboard: row });
}
...
export async function postDashboard(request: Request, env: Env): Promise<Response> {
  ...
  if (name.length > 100) return json(400, { error: "name_too_long" });
  ...
  try {
    ...
    if (/UNIQUE/i.test(msg)) return json(409, { error: "name_in_use" });
    return json(500, { error: "create_failed", detail: msg });
  }
}
...
export async function patchDashboard(
  ...
  if (body.name !== undefined) {
    if (typeof body.name !== "string" || !body.name.trim()) return json(400, { error: "invalid_name" });
    patch.name = body.name.trim().slice(0, 100);
  }
  if (body.layout !== undefined) {
    if (!Array.isArray(body.layout)) return json(400, { error: "invalid_layout" });
    // Light shape check — every entry must have a card_id string + size enum.
    for (const e of body.layout) {
      if (!e || typeof e !== "object") return json(400, { error: "invalid_layout_entry" });
      const ee = e as { card_id?: unknown; size?: unknown };
      if (typeof ee.card_id !== "string") return json(400, { error: "invalid_layout_entry" });
      if (!["sm","md","lg","xl"].includes(String(ee.size))) return json(400, { error: "invalid_layout_size" });
    }
    patch.layout_json = JSON.stringify(body.layout);
  }
  if (body.filters !== undefined) {
    if (typeof body.filters !== "object" || body.filters === null) return json(400, { error: "invalid_filters" });
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    if (/UNIQUE/i.test(msg)) return json(409, { error: "name_in_use" });
    return json(500, { error: "update_failed", detail: msg });
  }
}
...
export async function deleteOneDashboard(
  ...
  if (!result.ok) {
    ...
    if (result.reason === "is_default") return json(409, { error: "is_default_promote_first" });
  }
