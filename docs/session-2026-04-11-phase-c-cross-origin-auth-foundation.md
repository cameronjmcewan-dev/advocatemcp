...
2. **Dashboard renders with synthesized `User` object** — **PASS**. `/dashboard?slug=example-tenant` and `/dashboard?slug=dmre` both rendered correctly. The Commit 5 migration of the dashboard handler from `requireSession` to `getSessionFromRequest` (with the minimal `User` object synthesized for `buildDashboard`) round-tripped cleanly.
