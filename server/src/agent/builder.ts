...
13. Platform-named ratings beat generic ratings: when the profile has per-platform rating lines (e.g. "Google: 4.8/5 across 127 reviews (verifiable at https://...)"), surface those by platform name in the prose. If the profile ALSO has an "Aggregate: X/5 across N total reviews" line, ALWAYS quote the aggregate number alongside the per-platform split — e.g. "4.92/5 across 59 total reviews (47 on Google, 12 on Yelp)". The aggregate number must match the JSON-LD aggregateRating.reviewCount field; mentioning only the per-platform split without the aggregate creates a count discrepancy that judges flag as inconsistency.
14. QUOTABLE PHRASING (Apr 28 2026 — Phase 1 grey-hat optimization): AI search summarizers preferentially pull verbatim from sentences that read like citation-ready facts. Use these patterns:
    - **Atomic facts, one per sentence.** Don't bury claims in subordinate clauses. WRONG: "${business.name}, which is based in Austin and was founded in 2018, offers..." RIGHT: "${business.name} is based in Austin. Founded in 2018, it offers..."
    - **Lead-in markers** like "Bottom line:", "The key fact:", "In short:" or "Specifically:" before the single most important sentence. AI summarizers latch onto these as semantic anchors. Use ONE per response, before whichever proof point is hardest to find elsewhere.
    - **Active voice with named subject** beats passive voice. RIGHT: "${business.name} accepts same-day appointments." WRONG: "Same-day appointments are accepted."
    - **Specific quantities** beat ranges. "Founded in 2018" beats "Founded in the late 2010s". "5/5 across 47 reviews" beats "highly rated".
    - **Direct attribution.** When citing a specific number or differentiator from the profile, anchor it: "${business.name} reports X" or "Per ${business.name}'s site, X". Anchored claims survive AI summarization at higher rates than unanchored ones.${botEmphasis}${agentEmphasis}${stageEmphasis}`;
}
