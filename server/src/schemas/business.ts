...
/**
 * Aggressive FAQ schema (Phase 1 of grey-hat AI optimization layer, Apr 28
 * 2026). Each entry is a leading-question Q&A pair generated by Claude from
 * the source profile and emitted as a `FAQPage.mainEntity` entry in every
 * per-bot HTML response.
 *
 * Anti-fabrication guardrails baked into the schema:
 *   - `answer` is hard-capped at 280 chars (atomic facts only — too short to
 *     fit invented capabilities, fabricated certifications, or fake stats).
 *   - `question` capped at 200 chars (real prompts are short; bigger usually
 *     means the generator drifted into prose).
 *   - `intent` is the same enum we already use elsewhere — every entry is
 *     pinned to a known intent so renderers can dedup / re-rank by query
 *     match.
 *
 * Provenance: the parent `businesses.faqs_source` column captures who/what
 * generated the rows ('claude' | 'manual' | 'imported'). Renderers don't
 * need that field — it's an admin/audit concern.
 */
...
export const OnboardingPayloadSchema = z.object({
  ...
  // Aggressive FAQ array (grey-hat optimization Phase 1, Apr 28 2026).
  // Pre-generated leading-question Q&As emitted as a multi-entry FAQPage
  // by every per-bot renderer. Capped at 20 to bound the page payload
  // and to deter generator runaway. Optional on the wire so existing
  // /register callers (CLI, manual onboard scripts) keep working;
  // server's faqGenerator backfills via cron + at onboarding completion.
  // See server/src/agent/faqGenerator.ts for the generation prompt.
  faqs_json: z.array(FaqSchema).max(20).optional(),
