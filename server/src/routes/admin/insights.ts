...
adminInsightsRouter.get(
  ...
  (_req, res) => {
    ...
    const top      = topQueries(db, { limit: 25, days: 30 });
    const gaps     = profileGaps(db, { limit: 20 });
    const trends   = trendsByIndustry(db, { days: 14 });

    res.setHeader("Content-Type", "text/html; charset=utf-8");
    res.send(renderInsightsHtml({ overview, top, gaps, trends }));
  },
);
...
function renderInsightsHtml(data: RenderInput): string {
  const { overview: o, top, gaps, trends } = data;

  const topRows = top.length === 0
    ? `<tr><td colspan="5" class="empty">No queries logged yet.</td></tr>`
    : top.map((t, i) => `<tr>
        <td class="num">${i + 1}</td>
        <td>${esc(t.query_text)}</td>
        <td class="num">${t.count.toLocaleString()}</td>
        <td class="num">${t.unique_tenants.toLocaleString()}</td>
        <td>${esc(t.intent_v2 ?? "—")}</td>
      </tr>`).join("");
...
<h2>Top 25 queries (last 30 days)</h2>
<table>
  <thead><tr><th style="width:40px">#</th><th>Query</th><th style="width:80px">Count</th><th style="width:100px">Tenants</th><th style="width:140px">Intent_v2</th></tr></thead>
  <tbody>${topRows}</tbody>
</table>
